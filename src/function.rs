...
type BuiltinFunction = fn(&[Expression], &Environment) -> Result<(Expression, Environment)>;

pub fn plus(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
    get_numbers(exprs)
        .map(|nums| nums.iter().fold(0, |acc, n| acc + n))
        .map(|v| (IR::Integer(v).into(), env.clone()))
}

fn minus(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
...
fn multiple(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
    get_numbers(exprs)
        .map(|nums| nums.iter().fold(1, |acc, n| acc * n))
        .map(|v| (IR::Integer(v).into(), env.clone()))
}

fn divide(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
    let numbers = get_numbers(exprs)?;
...
fn make_qexpr(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
...
fn head_qexpr(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
...
fn tail_qexpr(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
...
fn join_qexprs(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
...
fn eval_qexpr(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
    if exprs.len() != 1 { return Err(error::make_argument_error("head", 1, exprs.len())) }
    match exprs[0].as_ir() {
...
fn define_symbol(exprs: &[Expression], env: &Environment) -> Result<(Expression, Environment)> {
    if exprs.len() < 2 {
